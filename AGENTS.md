...
`nh` is used under the hood. See `AGENTS_PROJECT.md` for available hosts and their package names.
...
| `wrappers` | `BirdeeHub/nix-wrapper-modules` | nixpkgs | binary wrapping |
...
`nh` helper is available via `den.lib.nh.denPackages`, which exposes flake apps as packages for each host.
...
| `test-driven-development` | Before writing implementation code for features/bugfixes. |
...
1. **Read AGENTS.md first** — understand project structure, conventions, and available tools.
2. **Invoke relevant skills** — if any skill might apply, invoke it before action.
3. **Brainstorm before doing** — use the `brainstorming` workflow before writing code.
4. **Follow existing patterns** — look at neighboring files for conventions.
5. **Verify before asserting** — run `nix flake check` before claiming completion.
6. **Ask, don't assume** — when unclear, ask the user rather than guessing.
7. **Commit when asked** — only commit or create PRs when explicitly requested.
8. **Document decisions** — save design specs to `docs/superpowers/specs/` following the brainstorming skill process.
