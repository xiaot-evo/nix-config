    ...
    in
    {
      ...
      homeManager =
        ...
        let
          ...
          cursorPkg = if (config.home.pointerCursor.package or null) != null then config.home.pointerCursor.package else pkgs.bibata-cursors;
          iconPkg = if (config.gtk.iconTheme.package or null) != null then config.gtk.iconTheme.package else pkgs.whitesur-icon-theme;
        in
        {
