...
2. 如果不可用，从锁定文件（`templates/ci/flake.lock`）中读取 `nix-effects` 节点
3. 使用 `builtins.fetchTarball` 从 GitHub archive URL 获取，并验证 `narHash`
4. 导入获取到的源码，传入 `lib` 作为参数
