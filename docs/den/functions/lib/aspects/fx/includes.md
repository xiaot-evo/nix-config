...
1. **守卫通过**：将 `meta.aspects` 作为子包含发射
1. **守卫失败或延迟**：调用 `deferConditionalHandler` 延迟条件
1. **延迟的条件**：稍后通过 `drainConditionalsHandler` 重新检查，此时上下文可能已经扩展
...
- [方面配置指南](../../../04-%E6%96%B9%E9%9D%A2%E9%85%8D%E7%BD%AE%E6%8C%87%E5%8D%97.md) — 条件方面的概念和用法
- [策略系统](../../../07-%E7%AD%96%E7%95%A5%E7%B3%BB%E7%BB%9F.md) — `den.lib.policy.when` 策略层条件判断
