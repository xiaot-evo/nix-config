...
2. **`__functor` 救援**：保留显式的 `__functor`（如 `den.batteries.forward`），防止被自由形式合并破坏
3. **provides 转发**：将 `provides` 的子键提升到方面顶层，使 `aspect.docker` 解析为 `aspect.provides.docker`
4. **合成 `_` 别名**：`_` 属性别名为 `provides`（通过 `lib.mkAliasOptionModule`），同时合成一个 `__functor` 包装的合成方面，暴露自由形式子键为 includes
